...
public static class CodeManager
{
	...
	public static string GetDeclaringType(bool showNamespace = false, int index = 0)
	{
		...
		if (st.FrameCount > 0)
		{
			try
			{
				...
				if (stt.GetFrame(0).GetMethod().Name == "GetMethodName" || stt.GetFrame(0).GetMethod().Name == "GetFunctionName")
	...
	[MethodImpl(MethodImplOptions.NoInlining)]
	public static string GetClassName(int index = 0)
	{
		return GetDeclaringType(false, index + 1);
	}
	...
	[MethodImpl(MethodImplOptions.NoInlining)]
	public static string GetMethodName(bool showClassName = true, bool showParams = false, int index = 0)
	{
		StringBuilder sb = new StringBuilder();
		...
		if (st.FrameCount > 0)
		{
			try
			{
				if (showCurrentTime)
				{
					sb.Append(System.DateTime.Now.ToString("yyyy/MM/dd tt hh:mm:ss.fff - "));
				}

                if (showClassName)
				{					
					sb.Append(GetDeclaringType(false, index + 1));
				}
				else
				{
					sb.Append("[");
	...
	/// <returns>The method name.</returns>
	[MethodImpl(MethodImplOptions.NoInlining)]
	public static string GetFunctionName(bool showClassName = true, bool showParams = false, int index = 0)
	{
		return GetMethodName(showClassName, showParams, index + 1);
	}
